...
public interface GameRepository extends JpaRepository<Game, Long> {
    ...
    @Query("SELECT COUNT(g) FROM Game g WHERE g.gameDate = :date AND g.status = 'FINISHED' AND NOT EXISTS (SELECT gi FROM GameInning gi WHERE gi.game = g)")
    int countFinishedWithoutInningsByDate(@Param("date") LocalDate date);
